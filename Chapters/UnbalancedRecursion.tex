...
Given a unrolling \emph{su} that synchronizes the functions, (EXT-BASE-EQUIV) prove conditional equivalence for the extended base case, whereas (MRC-STEP-EQUIV) prove the equivalence of the the step cases. We will now present our method to automatically generate such an unrolling, and then move on to describe (EXT-BASE-EQUIV) and (MRC-STEP-EQUIV).
...
The solution is using static analysis to determine in advance which recursive calls affect the current path and eliminate the rest if possible. The execution of this idea is not trivial and requires further thinking, as a call might be redundant on one frame and executed in a deeper frame.
